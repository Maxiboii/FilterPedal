...
struct Fifo
{
    ...
    {
        static_assert( std::is_same_v<T, std::vector<float>>, "prepare(numElements) should only be used when the Fifo is holding juce::AudioBuffer<float>");
        for( auto& buffer : buffers)
        {
            buffer.clear();
...
struct SingleChannelSampleFifo
{
    ...
    //==============================================================================
    int getNumCompleteBufferBuffersAvailable() const { return audioBufferFifo.getNumAvailableForReading(); }
